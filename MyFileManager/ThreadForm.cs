...
namespace MyFileManager
{
    public partial class ThreadForm : Form
    {
        ...
        {
            lblThreadsDescribe.Text = "Proces" + process.ProcessName + "Szczegóły wątków w są następujące：";
        ...
        private void StopThread()
        {
            if (lvwThread.SelectedItems.Count > 0)
            {
                ...
                {
                    MessageBox.Show("Nie można zamknąć tego wątku！", "Błąd", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
            else
            {
                MessageBox.Show("Wybierz wątek！", "Błąd", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
